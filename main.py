...
async def admin_info(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Lệnh check full thông tin người dùng cho Admin"""
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # MENU CẬP NHẬT: Thêm nút "👤 Tài khoản"
...
async def handle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) == 2 and parts[1].isdigit():
        code, amt = parts[0].upper(),from telegram import Update, InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardMarkup
from telegram.ext import ApplicationBuilder, CommandHandler, MessageHandler, ContextTypes, filters, CallbackQueryHandler
import sqlite3
from datetime import datetime, timedelta
import os
...
async def handle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "💰 Số dư":
        ...
        msg = (
            ...
            f"💰 Số dư: `{u[0]:,}đ`\n"
            f"👥 Đã mời: `{u[4]}` người\n"
            f"🏛 Ngân hàng: `{u[1] or 'Chưa có'}`\n"
            f"💳 STK: `{u[2] or 'Chưa có'}`\n"
            f"👤 Tên: `{u[3] or 'Chưa có'}`\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"💡 *Để cập nhật thông tin ngân hàng, hãy thực hiện lệnh rút tiền đầu tiên!*"
        )
...
async def rut(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if sub_money(uid, amount, "withdraw"):
            # Cập nhật thông tin bank vào DB để Admin có thể xem được qua /info
...
app.add_handler(CommandHandler("info", admin_info)) # Thêm lệnh /info
