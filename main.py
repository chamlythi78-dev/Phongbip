...
async def handle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "👤 Tài khoản":
        ...
        if check_mt('mt_nap') and uid != ADMIN_ID:
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith(("ok_", "no_")):
        ...
        if check_mt('mt_duaxe') and uid != ADMIN_ID:
...
if d == "menu_mines":
        if check_mt('mt_domin') and uid != ADMIN_ID:
    ...
    elfrom telegram import Update, InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardMarkup
from telegram.ext import ApplicationBuilder, CommandHandler, MessageHandler, ContextTypes, filters, CallbackQueryHandler
import sqlite3
from datetime import datetime, timedelta
import os
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith(("ok_", "no_")):
        ...
        if check_mt('mt_gomo') and uid != ADMIN_ID:
...
app.run_polling()
￼Enterif d.startswith("prep_mines_"):
        amt = int(d.split("_")[2])
        kb = [[InlineKeyboardButton("🚀 BẮT ĐẦU CHƠI", callback_data=f"start_mines_{amt}"), InlineKeyboardButton("🔙 Quay lại", callback_data="menu_mines")]]
        await q.edit_message_text(f"💣 **DÒ MÌN**\n💰 Cược: `{amt:,}đ`\n⚠️ Có 3 quả mìn ẩn trong 15 ô. Mở ô để nhân tiền!", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
